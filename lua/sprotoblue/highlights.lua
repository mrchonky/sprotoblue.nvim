local c = require("sprotoblue.colors")
local cfg = vim.g.sprotoblue_config
local util = require("sprotoblue.util")
...
function M.setup()
		...
		if color_name:sub(1, 1) == "$" then
			...
			if not color_name then
				vim.schedule(function()
					vim.notify(
						'sprotoblue.nvim: unknown color "' .. name .. '"',
						vim.log.levels.ERROR,
						{ title = "sprotoblue.nvim" }
					)
				end)
				return ""
			end
	...
	for group_name, group_settings in pairs(vim.g.sprotoblue_config.highlights) do
		vim.api.nvim_command(
			string.format(
				"highlight %s %s %s %s %s",
				group_name,
